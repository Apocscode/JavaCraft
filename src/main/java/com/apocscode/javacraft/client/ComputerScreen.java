package com.apocscode.javacraft.client;
...
/**
 * Minecraft Screen that renders the JavaCraft computer terminal.
 * Maps keyboard + mouse events into OS events.
 * Renders the TerminalBuffer as a grid of colored character cells.
 */
public class ComputerScreen extends Screen {

    private static final int CELL_WIDTH = 6;   // pixels per character cell
    private static final int CELL_HEIGHT = 9;  // pixels per character cell
    private static final int BORDER = 8;       // border around terminal area
    ...
    public ComputerScreen(JavaOS os) {
        super(Component.literal("JavaCraft Computer"));
        this.os = os;
    }
    ...
    protected void init() {
        ...
        termX = (this.width - gridW) / 2;
        termY = (this.height - gridH - HEADER_HEIGHT) / 2 + HEADER_HEIGHT;
    }
    ...
    public void render(GuiGraphics gfx, int mouseX, int mouseY, float partialTick) {
        ...
        gfx.drawString(this.font, headerText, termX, termY - HEADER_HEIGHT + 2, 0xAAAAAA, false);
        ...
        for (int y = 0; y < TerminalBuffer.HEIGHT; y++) {
            for (int x = 0; x < TerminalBuffer.WIDTH; x++) {
                ...
                if (c != ' ') {
                    int fgColor = TerminalBuffer.PALETTE[terminal.getFg(x, y)] & 0x00FFFFFF; // strip alpha for drawString
                    gfx.drawString(this.font, String.valueOf(c), px, py, fgColor, false);
                }
            }
        }
        ...
        if (terminal.isCursorBlink() && (System.currentTimeMillis() / 500) % 2 == 0) {
            ...
            gfx.fill(cx, cy + CELL_HEIGHT - 2, cx + CELL_WIDTH, cy + CELL_HEIGHT, 0xFFFFFFFF);
        }

        // Power indicator
        int indicatorColor = os.isRunning() ? 0xFF00FF00 : (os.isBooting() ? 0xFFFFAA00 : 0xFF555555);
        gfx.fill(termX + gridW - 6, termY - HEADER_HEIGHT + 4, 
                  termX + gridW - 2, termY - HEADER_HEIGHT + 8, indicatorColor);
    }
