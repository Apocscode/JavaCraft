    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (!level().isClientSide()) {
            if (ownerId == null) {
                ...
                player.sendSystemMessage(Component.literal("[JavaCraft Drone] Linked to you."));
            } else if (ownerId.equals(player.getUUID())) {
                player.sendSystemMessage(Component.literal(
                        "[JavaCraft Drone] Fuel: " + (fuelTicks / 20) + "s | Waypoints: " + waypoints.size()));
            } else {
                player.sendSystemMessage(Component.literal("[JavaCraft Drone] Not your drone."));
            }
        }
